...
playerWon player board = any isVictoryProj projs
  where
    ...
    isVictoryProj proj = (n ==) $ length $ filter (\cell -> cell == Full player) $ map (\coord -> board ! coord) proj
...
playerTurn game cellCoord
  | isCoordCorrect cellCoord && board ! cellCoord == Empty = checkGameOver $ switchPlayer $ game { gameBoard = board // [(cellCoord, Full $ player)] }
  | otherwise = game
  where
    board = gameBoard game
