...
public class ReverseHexAbc2 {
    static String form(String s) {
        ...
        if (res.toString().startsWith("0x") || res.toString().startsWith("0X")) {
            res = new StringBuilder(Integer.toString(Integer.parseUnsignedInt(res.substring(2).toString(), 16)));
        }
        for (int i = 0; i < res.length(); i++) {
            if (Character.isLetter(res.charAt(i)) || res.charAt(i) == '-') {
                continue;
            }
            res.setCharAt(i, (char) (res.charAt(i) - '0' + 'a'));
        }
        return res.toString();
    }
